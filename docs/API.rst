...
``czarpartyd`` features a full-fledged JSON RPC 2.0-based API, which allows
third-party applications to perform functions on the Czarparty network
...
By default, ``czarpartyd`` will listen on port ``4000`` (if on mainnet) or port ``14000`` (on testnet) for API
requests. 
...
Authentication
^^^^^^^^^^^^^^^
Also note that the ``czarpartyd`` API interface requires HTTP basic authentication to use. The username and password required
are stored in the ``czarpartyd.conf`` file, as ``rpc-user`` and ``rpc-password``, respectively. You can also modify
``rpc-host`` and ``rpc-port`` to change what interface and port number ``czarpartyd`` binds to from the defaults.

.. _examples:

Below we provide a few examples of using the ``czarpartyd`` API. Examples in other languages are welcome,
...
.. code-block:: python
    ...
    #Get all burns between blocks 280537 and 280539 where greater than .2 CZR was burned, sorting by tx_hash (ascending order)
    ...
    #Fetch all debits for > 2 XZR between blocks 280537 and 280539, sorting the results by quantity (descending order)
    payload = {
      "method": "get_debits",
      "params": {"filters": [{'field': 'asset', 'op': '==', 'value': "XZR"},
    ...
    #Send 1 XZR (specified in satoshis) from one address to another (you must have the sending address in your czarcoind wallet
...
With PHP, you can connect and query ``czarpartyd`` using the `JsonRPC <https://github.com/fguillot/JsonRPC>`__
...
26) string name of the asset, of at least 4 characters in length and not starting with 'A', or as 'CZR' or 'XZR' as appropriate. Examples are:
...
**NOTE:** XZR and CZR themselves are divisible assets, and thus are listed in satoshis.
...
The Czarparty API aims to be as simple and flexible as possible. To this end, it includes a straightforward
...
All ``create_`` API calls return an *unsigned raw transaction string*, hex encoded (i.e. the same format that ``czarcoind`` returns
...
- To return the transaction as an **OP_RETURN** transaction, specify ``opreturn`` for the ``encoding`` parameter.
  Note that as of ``czarcoind`` 0.9.0, not all Czarparty transactions are possible with OP_RETURN, due to the 40
  byte limit imposed by the ``czarcoind`` client in order for the transaction to be relayed on mainnet.
- To return the transaction as a **multisig** transaction, specify ``multisig`` for the ``encoding`` parameter.
    
    - If the source address is in the local ``czarcoind`` ``wallet.dat``. ``pubkey`` can be left as ``null``.
    - If the source address is *not* in the local ``czarcoind`` ``wallet.dat``, ``pubkey`` should be set to the hex-encoded
      public key.
- ``auto`` may also be specified to let ``czarpartyd`` choose here. Note that at this time, ``auto`` is effectively the same as
  ``multisig``.

- To return the Czarparty transaction encoded into arbitrary address outputs (i.e. pubkeyhash encoding), specify
  ``pubkeyhash`` for the ``encoding`` parameter. ``pubkey`` is also required to be set (as above, with ``multisig`` encoding)
  if the source address is not contained in the local ``czarcoind`` ``wallet.dat``. Note that this method is **not** recommended
...
- If the private key you need to sign the raw transaction is in the local ``czarcoind`` ``wallet.dat``, you should then call the
  ...
  API method.
- If the private key you need to sign the raw transaction is *not* in the local ``czarcoind`` ``wallet.dat``, you must first sign
  the transaction yourself (or, alternatively, you can call the ``sign_tx`` API method and specify
  the private key string to it, and ``czarpartyd`` will sign it for you). In either case, once you have the signed,
...
``balances``, ``credits``, ``debits``, ``bets``, ``bet_matches``, ``broadcasts``, ``czrpays``, ``burns``, 
...
**Notes:**
  ...
  * Please note that the ``get_balances`` API call will not return balances for CZR itself. It only returns balances
    for XZR and other Czarparty assets. To get CZR-based balances, use an existing system such as Insight, blockr.io,
    or blockchain.info.
...
Returns a list of all existing Czarparty assets. 

**Parameters:** None

**Return:**

  A list of existing Czarparty asset names.
...
Return message feed activity for the specified block index. The message feed essentially tracks all czarpartyd
...
Gets the current total quantity of XZR in existance (i.e. quantity created via proof-of-burn, minus quantity
...
**Return:** 
  ...
  The :ref:`quantity <quantitys>` of XZR currently in existance.
...
Gets some operational parameters for czarpartyd.
...
**Return:** 
  ...
  - **db_caught_up** (*boolean*): ``true`` if czarpartyd block processing is caught up with the Czarcoin blockchain, ``false`` otherwise.
  - **czarcoin_block_count** (**integer**): The block height on the Czarcoin network (may not necessarily be the same as ``last_block``, if ``czarpartyd`` is catching up)
  - **last_block** (*integer*): The index (height) of the last block processed by ``czarpartyd``
  - **czarpartyd_version** (*float*): The czarpartyd program version, expressed as a float, such as 0.5
  - **last_message_index** (*integer*): The index (ID) of the last message in the ``czarpartyd`` message feed
  - **running_testnet** (*boolean*): ``true`` if czarpartyd is configured for testnet, ``false`` if configured on mainnet.
  - **db_version_major** (*integer*): The major version of the current czarpartyd database
  - **db_version_minor** (*integer*): The minor version of the current czarpartyd database
...
**Parameters:**
  ...
  * **privkey (string):** The private key in WIF format to use for signing the transaction. If not provided,
    the private key must to be known by the ``czarcoind`` wallet.
...
Broadcast a signed transaction onto the Czarcoin network.
...
**Return:** 
  ...
  The created transaction's id on the Czarcoin network, or an error if the transaction is invalid for any reason.
...
**Parameters:**
  ...
  * **wager (integer):** The :ref:`quantity <quantitys>` of XZR to wager.
  * **counterwager (integer):** The minimum :ref:`quantity <quantitys>` of XZR to be wagered against, for the bets to match.
  ...
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``czarpartyd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``czarpartyd`` to automatically choose. 
  * **fee_per_kb (integer):** The fee per kilobyte of transaction data constant that ``czarpartyd`` uses when deciding on the dynamic fee to use (in satoshi). Leave as default unless you know what you're doing.
...
create_czrpay
^^^^^^^^^^^^^^
**create_czrpay(order_match_id, encoding='multisig', pubkey=null,
allow_unconfirmed_inputs=false, fee=null, fee_per_kb=10000)**

Create and (optionally) broadcast a CZRpay message, to settle an Order Match for which you owe CZR. 
...
Burn a given quantity of CZR for XZR (**only possible between blocks 278310 and 283810**).

**Parameters:**

  * **source (string):** The address with the CZR to burn.
  * **quantity (integer):** The :ref:`quantity <quantitys>` of CZR to burn (1 CZR maximum burn per address).
  * **encoding (string):** The encoding method to use, see :ref:`this section <encoding_param>` for more info.  
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``czarpartyd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``czarpartyd`` to automatically choose. 
...
**Parameters:**
  ...
  * **quantity_per_unit (integer):** The :ref:`quantity <quantitys>` of XZR rewarded per whole unit of the asset.
  * **encoding (string):** The encoding method to use, see :ref:`this section <encoding_param>` for more info.  
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``czarpartyd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``czarpartyd`` to automatically choose. 
...
**Parameters:**
  ...
  * **call_price (float):** The :ref:`price <floats>` per unit XZR at which the asset may be called back, on or after the specified call_date. Only valid for callable assets.
...
**Parameters:**
  ...
  * **fee_required (integer):** The miners' fee required to be paid by orders for them to match this one; in CZR; required only if buying CZR (may be zero, though). If not specified or set to ``null``, this defaults to 1% of the CZR desired for purchase.
  * **fee_provided (integer):** The miners' fee provided; in CZR; required only if selling CZR (should not be lower than is required for acceptance in a block).  If not specified or set to ``null``, this defaults to 1% of the CZR for sale. 
  * **encoding (string):** The encoding method to use, see :ref:`this section <encoding_param>` for more info.  
  * **pubkey (string):** The pubkey hex string. Required if multisig transaction encoding is specified for a key external to ``czarpartyd``'s local wallet. See :ref:`this section <encoding_param>` for more info.
  * **allow_unconfirmed_inputs (boolean):** Set to ``true`` to allow this transaction to utilize unconfirmed UTXOs as inputs.
  * **fee (integer):** If you'd like to specify a custom miners' fee, specify it here (in satoshi). Leave as default for ``czarpartyd`` to automatically choose. 
...
Send XZR or a user defined asset.
...
``bet``, ``broadcast``, ``czrpay``, ``burn``,  ``callback``, ``cancel``, ``dividend``, ``issuance``,
...
* **wager_quantity** (*integer*): The :ref:`quantity <quantitys>` of XZR to wager
* **counterwager_quantity** (*integer*): The minimum :ref:`quantity <quantitys>` of XZR to be wagered by the user to bet against the bet issuer, if the other party were to accept the whole thing
* **wager_remaining** (*integer*): The quantity of XZR wagered that is remaining to bet on
* **odds** (*float*): 
* **target_value** (*float*): Target value for Equal/NotEqual bet
* **leverage** (*integer*): Leverage, as a fraction of 5040
...
* **tx0_index** (*integer*): The Czarcoin transaction index of the initial bet
* **tx0_hash** (*string*): The Czarcoin transaction hash of the initial bet
* **tx0_block_index** (*integer*): The Czarcoin block index of the initial bet
...
* **target_value** (*float*): Target value for Equal/NotEqual bet  
* **leverage** (*integer*): Leverage, as a fraction of 5040
* **forward_quantity** (*integer*): The :ref:`quantity <quantitys>` of XZR bet in the initial bet
* **backward_quantity** (*integer*): The :ref:`quantity <quantitys>` of XZR bet in the matching bet
* **fee_multiplier** (*integer*): 
...
An object that matches a request to settle an Order Match for which CZR is owed:
...
* **burned** (*integer*): The :ref:`quantity <quantitys>` of CZR burned
* **earned** (*integer*): The :ref:`quantity <quantitys>` of XPC actually earned from the burn (takes into account any bonus quantitys, 1 CZR limitation, etc)
...
* **fee_provided** (*integer*): The miners' fee provided; in CZR; required only if selling CZR (should not be lower than is required for acceptance in a block)
* **fee_required** (*integer*): The miners' fee required to be paid by orders for them to match this one; in CZR; required only if buying CZR (may be zero, though)
...
* **tx0_index** (*integer*): The Czarcoin transaction index of the first (earlier) order
* **tx0_hash** (*string*): The Czarcoin transaction hash of the first order
* **tx0_block_index** (*integer*): The Czarcoin block index of the first order
...
An object that describes a specific send (e.g. "simple send", of XZR, or a user defined asset):
...
An object that describes a specific event in the czarpartyd message feed (which can be used by 3rd party applications
to track state changes to the czarpartyd database on a block-by-block basis).
...
* **category** (*string*): A string denoting the entity that the message relates to, e.g. "credits", "burns", "debits".
  The category matches the relevant table name in czarpartyd (see blocks.py for more info).
...
* **czrpays**: valid, invalid: {problem(s)}
...
This section documents any changes to the ``czarpartyd`` API, for version numbers where there were API-level modifications.
...
* DatabaseError: now a DatabaseError is returned immediately if the czarpartyd database is behind the backend, instead of after fourteen seconds
