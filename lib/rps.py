...
def cancel_rps (db, rps, status, block_index):
    ...
    util.credit(db, block_index, rps['source'], 'XZR', rps['wager'], action='recredit wager', event=rps['tx_hash'])
...
def update_rps_match_status (db, rps_match, status, block_index):
    ...
    if status in ['expired', 'concluded: tie']:
        ...
        util.credit(db, block_index, rps_match['tx0_address'], 'XZR',
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
        # Recredit tx1 address.
        util.credit(db, block_index, rps_match['tx1_address'], 'XZR',
                    rps_match['wager'], action='recredit wager', event=rps_match['id'])
        ...
        util.credit(db, block_index, winner, 'XZR',
...
def parse(db, tx, message):
    ...
    if status == 'open':
        ...
        rps_parse_cursor.execute('''SELECT * FROM balances \
                                    WHERE (address = ? AND asset = ?)''', (tx['source'], 'XZR'))
    ...
    if status == 'open':
        util.debit(db, tx['block_index'], tx['source'], 'XZR', wager, action="open RPS", event=tx['tx_hash'])
    ...
    for rps_match in cursor.fetchall():
        ...
        if new_rps_match_status == 'expired':
            ...
            for rps in matched_rps:
                ...
                # Re-debit XZR refund by close_rps_match.
                util.debit(db, block_index, rps['source'], 'XZR', rps['wager'], action='reopen RPS after matching expiration', event=rps_match['id'])
                # Rematch
