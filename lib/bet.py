...
from . import (util, config, czarcoin, exceptions, util)
...
def cancel_bet (db, bet, status, block_index):
    ...
    util.credit(db, block_index, bet['source'], config.XZR, bet['wager_remaining'], action='recredit wager remaining', event=bet['tx_hash'])
...
def cancel_bet_match (db, bet_match, status, block_index):
    ...
    util.credit(db, block_index, bet_match['tx0_address'], config.XZR,
                bet_match['forward_quantity'], action='recredit forward quantity', event=bet_match['id'])

    # Recredit tx1 address.
    util.credit(db, block_index, bet_match['tx1_address'], config.XZR,
...
def parse (db, tx, message):
    ...
    if status == 'open':
        ...
        bet_parse_cursor.execute('''SELECT * FROM balances \
                                    WHERE (address = ? AND asset = ?)''', (tx['source'], config.XZR))
    ...
    if status == 'open':
        util.debit(db, tx['block_index'], tx['source'], config.XZR, wager_quantity)
    ...
    for tx0 in bet_matches:
        ...
        if tx0_inverse_odds > tx1_odds:
            ...
            if tx0_wager_remaining <= 0 or tx0_counterwager_remaining <= 0:
                ...
                tx0_status = 'filled'
                util.credit(db, tx1['block_index'], tx0['source'], config.XZR, tx0_wager_remaining, event=tx1['tx_hash'], action='filled')
            bindings = {
            ...
            if tx1['block_index'] >= 292000 or config.TESTNET:  # Protocol change
                if tx1_wager_remaining <= 0 or tx1_counterwager_remaining <= 0:
                    # Fill order, and recredit give_remaining.
                    tx1_status = 'filled'
                    util.credit(db, tx1['block_index'], tx1['source'], config.XZR, tx1_wager_remaining, event=tx1['tx_hash'], action='filled')
            # tx1
            bindings = {
