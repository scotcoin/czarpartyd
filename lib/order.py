...
# Filled orders may not be re‐opened, so only orders not involving CZR (and so
...
def exact_penalty (db, address, block_index, order_match_id):
    # Penalize addresses that don’t make CZR payments. If an address lets an
    # order match expire, expire sell CZR orders from that address.
    cursor = db.cursor()

    # Orders.
    bad_orders = list(cursor.execute('''SELECT * FROM orders \
                                        WHERE (source = ? AND give_asset = ? AND status = ?)''',
                                     (address, config.CZR, 'open')))
    ...
    if not (block_index >= 314250 or config.TESTNET):   # Protocol change.
        ...
        bad_order_matches = list(cursor.execute('''SELECT * FROM order_matches \
                                         ...
                                         (address, config.CZR, address, config.CZR, 'pending')))
...
def cancel_order (db, order, status, block_index):
    ...
    if order['give_asset'] != config.CZR:    # Can’t credit CZR.
...
def cancel_order_match (db, order_match, status, block_index):
    ...
    if tx0_order['status'] in ('expired', 'cancelled'):
        ...
        if order_match['forward_asset'] != config.CZR:
        ...
        if tx0_order['get_asset'] == config.CZR and (block_index >= 297000 or config.TESTNET):    # Protocol change.
    ...
    if tx1_order['status'] in ('expired', 'cancelled'):
        ...
        if order_match['backward_asset'] != config.CZR:
        ...
        if tx1_order['get_asset'] == config.CZR and (block_index >= 297000 or config.TESTNET):    # Protocol change.
    ...
    if block_index >= 313900 or config.TESTNET:  # Protocol change.
        if tx0_order['status'] == 'expired' and order_match['forward_asset'] == config.CZR:
            exact_penalty(db, order_match['tx0_address'], block_index, order_match['id'])
        if tx1_order['status'] == 'expired' and order_match['backward_asset'] == config.CZR:
...
def validate (db, source, give_asset, give_quantity, get_asset, get_quantity, expiration, fee_required, block_index):
    ...
    if give_asset == config.CZR and get_asset == config.CZR:
        problems.append('cannot trade {} for itself'.format(config.CZR))
    ...
    if give_asset not in (config.CZR, config.XZR) and not cursor.fetchall():
    ...
    if get_asset not in (config.CZR, config.XZR) and not cursor.fetchall():
...
def compose (db, source, give_asset, give_quantity, get_asset, get_quantity, expiration, fee_required):
    ...
    if give_asset != config.CZR and (not balances or balances[0]['quantity'] < give_quantity):
...
def parse (db, tx, message):
    ...
    if status == 'open':
        if give_asset != config.CZR:  # No need (or way) to debit CZR.
...
def match (db, tx, block_index=None):
    ...
    for tx0 in order_matches:
        ...
        # Make sure that that both orders still have funds remaining (if order involves CZR, and so cannot be ‘filled’).
        if tx0['give_asset'] == config.CZR or tx0['get_asset'] == config.CZR: # Gratuitous
        ...
        if tx0_price > tx1_inverse_price:
            ...
            if block_index >= 313900 or config.TESTNET: # Protocol change.
                min_czr_quantity = 0.001 * config.UNIT  # 0.001 CZR
                if (forward_asset == config.CZR and forward_quantity <= min_czr_quantity) or (backward_asset == config.CZR and backward_quantity <= min_czr_quantity):
                    logging.debug('Skipping: below minimum {} quantity'.format(config.CZR))
                    continue
            ...
            if block_index >= 286500 or config.TESTNET: # Protocol change. Deduct fee_required from fee_provided_remaining, etc., if possible (else don’t match).
                ...
                elif tx1['give_asset'] == config.CZR:
            ...
            if config.CZR in (tx1['give_asset'], tx1['get_asset']):
                status = 'pending'
            else:
                status = 'completed'
                # Credit.
            ...
            # Debit the order, even if it involves giving czarcoins, and so one
            ...
            if tx0_give_remaining <= 0 or (tx0_get_remaining <= 0 and (block_index >= 292000 or config.TESTNET)):    # Protocol change
                if tx0['give_asset'] != config.CZR and tx0['get_asset'] != config.CZR:
            ...
            if tx1_give_remaining <= 0 or (tx1_get_remaining <= 0 and (block_index >= 292000 or config.TESTNET)):    # Protocol change
                if tx1['give_asset'] != config.CZR and tx1['get_asset'] != config.CZR:
                    # Fill order, and recredit give_remaining.
                    tx1_status = 'filled'
...
def expire (db, block_index):
    ...
    # Expire orders and give refunds for the quantity give_remaining (if non-zero; if not CZR).
    cursor.execute('''SELECT * FROM orders \
    ...
    # Expire order_matches for CZR with no CZR.
    cursor.execute('''SELECT * FROM order_matches \
